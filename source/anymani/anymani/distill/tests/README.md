...
当前硬门槛针对 RTX 5070 Ti、batch size $B=4096$、单结构组下隐式主线的完整在线 retained geometry encoder。计时从 GPU-resident $q$、screw/anchors、raw home geometry 与 topology 开始；joint limits 只服务 q 采样，不进入 encoder。计时覆盖 adapter、集合聚合和 backbone final-norm unified $Z$，排除磁盘/CPU cache materialization、host-to-device copy、decoder、policy head、Isaac Sim 与 `env.step`。未来若激活解析直接候选，同一门槛从 GPU-resident $q$、cached local support points、topology/poses 开始，并必须计入批量 FK/刚体点变换。

快速 suite 使用 20 次预热和 50 次 CUDA Event 计时，要求 p95 不超过 40 ms，并报告 median 与 max。200 次深度 profile 只用于候选比较、模块拆时和峰值显存分析，不进入日常测试。

```bash
pytest source/anymani/anymani/distill/tests/performance/test_canonical_retained_geometry_encoder_latency.py -m performance -q -s
```

同一文件另行报告正式 SSL 微批次下 disposable density/sensitivity readers 的前向参数量、延迟与峰值显存，但不把它们计入 retained 40 ms 门槛。
