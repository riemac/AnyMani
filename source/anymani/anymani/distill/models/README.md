...
contracts 已检查三种 graph-bias 的精确 lookup 与相加、末距离桶仍参与全连接 attention、anchor permutation、$SO(2)$ rewrite、joint-sign parity、variable sigma、sigma detach、每层 FiLM、跨结构 output/gradient 和 retained/disposable keys。真实 LEAP integration 已完成 encoder、readers、三项 method objective 与普通参数 backward。
