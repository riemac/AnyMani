...
geometry SSL 的可运行入口为 ``python -m anymani.distill.ssl.pretrain``，普通 CLI flags 描述一次运行，schema 7 Python preset 提供方法与默认配置。IL 尚未建立 trainer。geometry SSL 保持 task-free PyTorch/Warp 依赖边界，RL wrapper 与 Isaac Sim runtime 由对应阶段入口加载。
