...
与必要结构 metadata，输出供 retained $z^{(0)}/z^{(1)}$ heads、representation decoder、
policy/value head 或 distillation objective 使用的 latent。它不解析资产、不生成 target，也不
...
PALM/JOINT/TIP entity 与 surface owner 同索引，动作路由由 JOINT 角色决定。input adapter、backbone 与
$z^{(0)}/z^{(1)}$ heads 一起由 SSL checkpoint 迁入 PPO，并默认 full fine-tune。RTX 5070 Ti、$B=4096$、
