...
def init_adr_noise_buffers(env):
...
def update_adr_noise_buffers(env, env_ids):
    num_ids = env_ids.shape[0]
    env.object_pos_bias_width[env_ids, 0] = env.leap_adr.get_custom_param_value("object_state_noise", "object_pos_bias") * torch.rand(num_ids, device=env.device)
    env.object_rot_bias_width[env_ids, 0] = env.leap_adr.get_custom_param_value("object_state_noise", "object_rot_bias") * torch.rand(num_ids, device=env.device)
    ...
    env.object_pos_noise_width[env_ids, 0] = env.leap_adr.get_custom_param_value("object_state_noise", "object_pos_noise") * torch.rand(num_ids, device=env.device)
    env.object_rot_noise_width[env_ids, 0] = env.leap_adr.get_custom_param_value("object_state_noise", "object_rot_noise") * torch.rand(num_ids, device=env.device)

    env.robot_bias_width[env_ids, 0] = env.leap_adr.get_custom_param_value("robot_state_noise", "robot_bias") * torch.rand(num_ids, device=env.device)
    env.robot_bias[env_ids, 0] = env.robot_bias_width[env_ids, 0] * (torch.rand(num_ids, device=env.device) - 0.5)
    env.robot_state_noise_width[env_ids, 0] = env.leap_adr.get_custom_param_value("robot_state_noise", "robot_noise") * torch.rand(num_ids, device=env.device)
    env.hand_action_noise_width[env_ids, 0] = env.leap_adr.get_custom_param_value("robot_action_noise", "hand_noise") * torch.rand(num_ids, device=env.device)


def apply_object_wrench(env, obj, object_name):
    body_ids = None
    env_ids = None
    ...
    forces = max_force * 2 * (torch.rand(env.num_envs, num_bodies, 3, device=env.device) - 0.5)
    torques = max_torque * 2 * (torch.rand(env.num_envs, num_bodies, 3, device=env.device) - 0.5)
