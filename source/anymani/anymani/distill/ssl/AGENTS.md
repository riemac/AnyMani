...
```text
ssl/
├── experiment.py                    schema-9 pure-pretrain 根配置与 façade
├── post_training.py                 独立 evaluation 根配置与 façade
...
├── source_cache.py                  cache status/audit/prune CLI
├── admission.py                     fixed-batch parity/precision admission CLI
├── evaluate.py                      显式 checkpoint evaluation CLI
├── checkpoint.py                    通用 method/optimizer/trainer state 容器
├── experiments/
│   ├── __init__.py                  显式实验 registry
│   └── geometry_ssl_multitask_representation_v0_7_5.py
...
快照至少包含 data、method、representation/model、objective、trainer 和 run；evaluation 从同一快照导出。
`EXPERIMENT` 是训练主配置，事后阶段不属于训练生命周期。
...
训练入口不自动调用 evaluation、PCA 或 best selection；训练完成自动发布 retained encoder。实验快照若将
...
若 incomplete run 之后只进行了已验证的源码修复，resume 可显式增加 `--allow-worktree-change`。该开关
只放行已验证源码修复带来的 code revision 与 dirty worktree fingerprint 变化，仍严格比较 scientific
config、package version、objective/FairGrad、参数分区、source identity 和 dirty 状态；新 formal run
不应使用该开关。
...
```bash
/home/hac/isaac/IsaacLab/isaaclab.sh -p -m anymani.distill.ssl.prepare_sources --config geometry_ssl_multitask_representation_v0_7_5 --device cuda:0
```

`backup.sh` 只负责训练，不包含 prepare、status、validate 或 evaluate 子命令；source preparation 是否
发生由实验配置和 pretrain runtime 内部决定。

正式训练前的固定形状证据由 admission CLI 生成；`streamed-parity` 比较完整 512-pair update 与 8 个
64-pair units，`precision` 比较同一 teacher batch 的 FP32/BF16 执行路径。已完成的本轮报告位于
`logs/ssl/_admission/`；v1/v2 source parity 报告在清理旧 v1 cache 前完成并保留。
...
和训练 checkpoint。evaluation runtime 只消费已完成的 schema-9 full checkpoint，不回写训练目录。
...
pure pretrain 不生成 best checkpoint；完成后自动发布 schema-5 standalone retained artifact，full checkpoint
服务 SSL resume 与事后 evaluation。
...
cache 维护和 fixed-batch admission 的入口分别为 `python -m anymani.distill.ssl.source_cache status|audit|prune`
与 `python -m anymani.distill.ssl.admission streamed-parity|precision`；两者都必须使用当前 v0.7.5
实验快照和 IsaacLab Python 环境。
