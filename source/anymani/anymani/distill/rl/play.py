r"""AnyMani ``distill.rl`` 的 rl_games checkpoint 回放入口。

本入口和 ``anymani.distill.rl.train`` 成对存在：``rl/train.py`` 负责 rl_games PPO 训练，
...
parser.add_argument(
    "--rl_games_strict", action="store_true", default=False, help="Require local rl_games v1.6.5 commit."
)
