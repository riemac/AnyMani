r"""TODO: 手资产银行（Hand Asset Bank）的集合与选择 scaffold。
...
发现、解析并选择一组 hand containers”。单个 container 对外呈现一个“作伪”的
虚拟标准 bundle 视图：
...
`virtual path <-> real path` 的双射。下游 `tasks/gm` 和 `distill` 只消费这个
标准视图，再各自适配 IsaacLab spawn、geometry observation、训练 manifest 等语义。

第一版实现边界（本文件当前只落 scaffold，不实现）：

- `source_mode="post_mutate"` 是第一条落地路径；
- `source_mode="pre_made"` / `"mixed"` 先保留接口，resolve 时应显式报
  `NotImplementedError`；
...
- 文件 IO、XML/YAML 解析、采样算法和虚拟路径映射均留给后续实现，不在 scaffold
  中提前写死。
...
class HandBank(AssetBank):
    ...
    def resolve(self) -> HandSelection:
        ...
        Raises:
            NotImplementedError: 当前只落 scaffold，后续再实现 post-mutate explicit / sample / all。
        """
    ...
    def discover(self) -> tuple[HandContainer, ...]:
        ...
        Raises:
            NotImplementedError: 当前只落 scaffold，后续实现 post-mutate run 目录候选发现。
        """
    ...
    def select(self, candidates: tuple[HandContainer, ...]) -> HandSelection:
        ...
        Raises:
            NotImplementedError: 当前只落 scaffold，后续实现稳定排序、seed sampling 与 selection metadata。
        """
