r"""AnyMani ``distill.rl`` 的 rl_games PPO 训练入口。

本入口统一服务 single-asset baseline 与 canonical five-mother masked PPO。它不定义环境语义：
`tasks/gm` 仍然拥有 scene、obs、action、reward、reset、termination；这里仅负责启动 Isaac Sim、
加载 distill registry 中的 rl_games YAML、物化 canonical typed geometry evidence、固定本地
rl_games backend、写出训练复现实验目录并运行 PPO。
...
parser.add_argument("--rl_games_strict", action="store_true", default=False)
...
def _dump_agent_cfg(path: str, agent_cfg: dict) -> None:
    r"""写可复现 YAML，但不把进程内 tensor evidence/provider object 交给序列化器。"""
...
class RslStyleIsaacAlgoObserver(OneShotIsaacAlgoObserver):
    ...
    def _print_rsl_style_summary(
        self, stats: dict[str, float | int | None], episode_summary: dict[str, float], width: int = 80
    ) -> None:
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict) -> None:
    ...
    agent_yaml_path = os.path.join(log_root_path, log_dir, "params", "agent.yaml")
    _dump_agent_cfg(agent_yaml_path, agent_cfg)
    ...
    record_optional_rl_phase("environment_construct", "complete", num_envs=int(env.unwrapped.num_envs))
    ...
    _dump_agent_cfg(agent_yaml_path, agent_cfg)  # masked run 补写 dataset/manifest/provider identity

    if agent_cfg["params"]["algo"]["name"] == "anymani_masked_ppo":
        runner = AnyManiMaskedRunner(_make_isaac_algo_observer(agent_cfg))
    else:
    ...
    record_optional_rl_phase("ppo_train", "start", max_epochs=agent_cfg["params"]["config"]["max_epochs"])
    runner.run(runner_args)
    record_optional_rl_phase("ppo_train", "complete")
    env.close()
