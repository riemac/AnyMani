...
def test_mvp80_yaml_locks_stratified_budget_dual_lr_and_fp32_policy() -> None:
    ...
    assert config["contextual_learning_rate"] == 1.0e-4
    ...
    assert config["advantage_normalization_scope"] == "global"
    assert config["value_bootstrap"] is False
    ...
    assert config["gradient_probe_frequency"] == 500
    assert config["full_gradient_shadow_frequency"] == 0
    assert config["gpu_memory_fraction_limit"] == 0.85
    assert config["gpu_driver_free_memory_bytes_min"] == 2 * 1024**3
