r"""Geometry SSL Multitask Representation v0.7.5 的冻结实验快照。
...
共 1024 optimizer updates 和 8 个完整 8192-asset catalog cycles。训练按 8-asset stream unit
增量构造 compact source artifact，并在每个 catalog cycle 保存 immutable full checkpoint；每个
epoch 另保存滚动 recovery。训练完成固定发布 ``last.pt`` 与 ``retained_encoder.pt``，held-out
evaluation 只消费用户显式指定的 checkpoint，不选择或回写训练产物。
...
RUN_CFG = PretrainRunCfg(
    output_dir="logs/ssl",
    experiment_name="geometry_ssl_multitask_representation_v0_7_5",
    seed=20260813,
...
EVALUATION_EXPERIMENT = EmbodimentEvaluationCfg(
    ...
    run=EvaluationRunCfg(experiment_name="geometry_ssl_multitask_representation_v0_7_5_evaluation"),
)
