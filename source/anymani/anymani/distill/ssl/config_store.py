...
"""Geometry SSL schema-9 正式实验快照的 Hydra 根配置名。"""
...
def _register_preset(store: ConfigStore, preset: ExperimentPreset) -> tuple[str, str | None]:
    r"""把一个 registry entry 映射为 Hydra 的 pretrain/evaluation 名称。"""
...
def compose_pretrain_cfg(
    ...
    r"""从 ConfigStore 恢复 schema 9 pure-pretrain 根配置，不读取 distill YAML。
