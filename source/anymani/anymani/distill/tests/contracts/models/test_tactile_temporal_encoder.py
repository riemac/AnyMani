...
def test_per_joint_tcn_shares_parameters_without_cross_joint_temporal_mixing() -> None:
    r"""`[B,30,16,4]`只沿时间编码；joint间交互留给后续graph policy adapter。"""

    encoder = PerJointTactileTemporalEncoder(joint_count=16, frame_dim=4, latent_dim=64)
    history = torch.ones(2, 30, 16, 4)  # `[B,T,N_J,F]` oldest-to-latest raw local history
    active = torch.ones(2, 16, dtype=torch.bool)
    active[0, 7:] = False  # 第一资产只有7个active joints

    baseline = encoder(history, active)
    perturbed = history.clone()
    perturbed[:, :, 0, 0] += 1.0  # 只改变joint-0的$q/\pi$历史
    changed = encoder(perturbed, active)

    assert baseline.shape == (2, 16, 64)
    assert not torch.equal(changed[:, 0], baseline[:, 0])
    torch.testing.assert_close(changed[:, 1], baseline[:, 1])  # TCN不跨joint通信
    assert torch.count_nonzero(baseline[0, 7:]) == 0  # ghost temporal latent保持精确零
    assert encoder.encoder.conv1.weight.data_ptr()  # 只有一套共享TCN参数，不实例化16个modules


def test_per_joint_history_stack_preserves_order_and_joint_independence() -> None:
    r"""Ordered stack应保留全部30帧列位置，同时不在时序encoder内跨JOINT通信。"""

    torch.manual_seed(23)
    encoder = PerJointHistoryStackEncoder(joint_count=16, frame_dim=4, latent_dim=32)
    history = torch.randn(2, 30, 16, 4)
    active = torch.ones(2, 16, dtype=torch.bool)
    active[0, 7:] = False
    baseline = encoder(history, active)
    oldest_changed = history.clone()
    oldest_changed[:, 0, 0, 0] += 1.0
    latest_changed = history.clone()
    latest_changed[:, -1, 0, 0] += 1.0

    assert baseline.shape == (2, 16, 32)
    assert not torch.equal(encoder(oldest_changed, active)[:, 0], baseline[:, 0])
