...
probe 使用唯一 `anymani.distill.rl.train` 入口、绑定 tasks-owned single-asset env cfg、
...
TRAIN_ENTRY_PATH = Path(anymani.distill.rl.__file__).resolve().with_name("train.py")
"""统一 RL 训练入口源码路径；从被测 package 定位，不依赖测试目录层级。"""

PLAY_ENTRY_PATH = Path(anymani.distill.rl.__file__).resolve().with_name("play.py")
"""统一 RL checkpoint 回放入口源码路径；从被测 package 定位，不依赖测试目录层级。"""
